...
class sms:
	...
	def banner(self):
		os.system('clear')
		time.sleep(2);
		wr(f"""{b}_____ _____                              ___ ____
|_   _|_   _|__ _ __ _ __ ___  _   ___  _|_ _|  _ \  \

  | |   | |/ _ \ '__| '_ ` _ \| | | \ \/ /| || | | |
{p}  | |   | |  __/ |  | | | | | | |_| |>  < | || |_| |
  |_|   |_|\___|_|  |_| |_| |_|\__,_/_/\_\___|____/
{k}══════════════════════════════════════
{m}[{h}+{p}]{b}Author  : {p} Billal
{m}[{h}+{p}]{b}Github  : {p} https://github.com/TTermuxID
{m}[{h}+{p}]{b}Youtube : {p} Tutor Termux ID
{k}══════════════════════════════════════
		""")
...
try:
	...
	while True:
		...
		if plh.lower() == 'y':
			...
			print(f"{b}Jangan {p}Lupa {b}Subscribe {p}Tutor Termux ID");time.sleep(1);
			print(f"{m}Exit")
			sys.exit()
